...
class AnonGPT(loader.Module):
    """
    🧠 AnonGPT | Zetta AI
    🌒 v1.0 | Beta, please feed back

    Модуль для развлечения в боте @AnonRuBot.
    Модель ИИ подключается в чат, и с подкреплением системной роли ведет диалоги.
    Вы можете получить шаблон промта для заполения, и придумать характер для ИИ.
    История диалога сбрасывается каждый раз, когда собеседник отключается.
    Заходите в наш тгк: @hikkagpt и присылайте интересные переписки.

    📝 Команды:
    `.anon on/off` - включить/выключить модуль
    `.anonclear` - сбросить историю диалога в ручную
    `.anonrole <текст>` - установить роль для ИИ
    `.anonel` - шаблон для установки роли. (Важно)
    """
    ...
    async def anonelcmd(self, message):
        ...
        template = """🎭 Вам предоставлен шаблон для заполнения, что бы создать характер для ИИ.
